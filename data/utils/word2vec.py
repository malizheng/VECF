...
input_file = folder+'word_data.gz'   # sys.argv[1]   gz file
col = 0 #int(sys.argv[2])
outfile_embedding = folder+'word_emb_tmp.pkl'  #sys.argv[3]
outfile_dict = folder+'word_id_dict_tmp' #sys.argv[4]
outfile_dict_rev = folder+'id_word_dict_tmp' #sys.argv[5]

embedding_size = 64  #int(sys.argv[6])  # Dimension of the embedding vector.
num_steps = 1000001
...
words = []
for l in gFile:
    wd = [w for w in l.strip().split('$$$$&&&&')]
    words.extend(wd)

gFile.close()

print('Data size', len(words))
print('vocabu size', len(set(words)))

# Step 2: Build the dictionary and replace rare words with UNK token.
vocabulary_size = 50000000000
...
with tf.Session(graph=graph, config=cfg) as session:
    ...
    print('vocabu size', len(final_embeddings))
